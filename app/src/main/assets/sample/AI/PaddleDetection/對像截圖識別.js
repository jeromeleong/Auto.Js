...
let clickButtonWindow = floaty.rawWindow(
  <vertical>
    <button id="captureAndOd" text="截图识别" />
...
// 点击识别
clickButtonWindow.captureAndOd.click(function () {
  result = []
  ui.run(function () {
...
window.canvas.on('draw', function (canvas) {
  ...
  if (result && result.length > 0) {
    for (let i = 0; i < result.length; i++) {
      let odResult = result[i]
      drawRectAndText(odResult.words + ' #信心:' + odResult.confidence.toFixed(2), odResult.bounds, '#00ff00', canvas, paint);
    }
  }
})
